...
`template_sha256` — хеш `scripts/templates/prompt.txt`, которым собран этот промпт. Сверка
промпта байт-в-байт идёт только для сцен, собранных текущим шаблоном: правка шаблона не
должна задним числом объявлять прежние промпты неверными, иначе закоммиченный промпт
перестаёт быть тем, из которого получена картинка.
